...
let LoopClass = class {
  ...
  set_event(){
    var array_2 = ["RA_ITEM_PREV","RA_ITEM_NEXT","RA_PLAY_START","RA_PLAY_PAUSE","RA_PLAY_STOP","RA_PLAY_RND"];
...
let PL_statuClass = class {
  ...
  set_event(){
    document.getElementById("bt_pl_1").addEventListener('click', event => {
      ...
      if (this.set && this.play)  socket_send(api_request(0, "SOKCET", [{"n":"RA_PLAY_LB", "v":""}],  ["list_lb", "loop"]));
      else                        socket_send(api_request(0, "SOKCET", [{"n":"RA_PLAY_PL", "v":""}],  ["list_pl", "loop"]));
    });     
  }
  socketParse(json) {

...
function LoopClass_btEvent(e){
  let array_2 = ["RA_ITEM_PREV","RA_ITEM_NEXT","RA_PLAY_START","RA_PLAY_PAUSE","RA_PLAY_STOP","RA_PLAY_RND"];
  let array_1 = ["prev","next","play","pause","stop","rnd"];      
  for (var i = 0; i < array_1.length; i++) {
    if (e.target.textContent==array_1[i]) 
      socket_send(api_request(0, "SOKCET", [{"n":array_2[i],  "v":""}], [""]));}    
}
let LoopClass = class {
  constructor(v) {
    this.play     = null;
  ...
  set_event(){
    let array_1 = ["prev","next","play","pause","stop","rnd"];
    let i = 0;    
    document.querySelectorAll('.btn-ap').forEach( (e) => {e.textContent = array_1[i];i++;});
...
let PL_statuClass = class {
  constructor(v1, v2) {
    ...
    document.getElementById("bt_pl_1").addEventListener('click', event => {
      ...
      if (this.set && this.play) 
        socket_send(api_request(0, "SOKCET", [{"n":"RA_PLAY_LB", "v":""}],  ["list_lb", "loop"]));
      else
        socket_send(api_request(0, "SOKCET", [{"n":"RA_PLAY_PL", "v":""}],  ["list_pl", "loop"]));
    });    
  }
  socketParse(json) {
